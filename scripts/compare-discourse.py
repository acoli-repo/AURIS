...
for src,tgt in zip(args.SRC,args.TGT):
	...
	for id,tgt in sid2tgt.items():
		if id in tid2tgt:
			...
			if not srel in srel2trel2freqUTT: srel2trel2freqUTT[srel]={}
			if not trel in srel2trel2freqUTT[srel]: srel2trel2freqUTT[srel][trel]=0
			srel2trel2freqUTT[srel][trel]+=1
			if srel==trel:
				UTT_SENSE_TP+=1
			UTT_SENSE_TOTAL+=1
...
UTT_SENSE_ACC=UTT_SENSE_TP/UTT_SENSE_TOTAL

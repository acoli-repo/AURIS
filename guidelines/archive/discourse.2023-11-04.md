...
- if the current NUC is a copula verb, retrieve the marker from its syntactic head, otherwise from dependents of the NUC
- if the head of the current NUC is an attribution verb, consider in the following the dependents both of NUC and its syntactic head
- given a list of known discourse markers (and alternative lexicalizations), if any of these are direct dependents, choose the first
- if none is found: choose the first `cc` argument
- if none is found: choose the first `discourse` argument
- if none is found: choose the first `advmod` argument
- if none is found: choose the first `advcl` as discourse marker candidate (marked with `?`)
- if none is found, propose no discourse marker

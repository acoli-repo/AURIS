...
The AURIS guidelines have been compiled from existing guidelines whose original creators are attributed along with the compilator. 
Note that we do not track all possible changes within this document, but instead, we provide the modified guidelines along with the original files in the same format, so that changes can be tracked automatically (e.g., using the command line tool `diff`).
